...
public class HelloTest extends JerseyTest {
    ...
    public void testHelloWorld() throws Exception {
        URL getUrl = UriBuilder.fromUri(getBaseUri()).path(Application.ROOT_PATH).build().toURL();
    ...
    public void testConnection() {
        Response response = target().path(Application.ROOT_PATH).request("text/plain").get();
        assertEquals(200, response.getStatus());
    }

    @Test
    public void testClientStringResponse() {
        String s = target().path(Application.ROOT_PATH).request().get(String.class);
        assertEquals(HelloResource.CLICHED_MESSAGE, s);
    }
    ...
    public void testAsyncClientRequests() throws InterruptedException {
        ...
        for (int i = 0; i < REQUESTS; i++) {
            final int id = i;
            target().path(Application.ROOT_PATH).request().async().get(new InvocationCallback<Response>() {
                @Override
                public void completed(Response response) {
                    try {
                        final String result = response.readEntity(String.class);
                        assertEquals(HelloResource.CLICHED_MESSAGE, result);
                    } finally {
                        latch.countDown();
                    }
                }
        ...
        Logger.getLogger(HelloTest.class.getName()).info("Executed in: " + (toc - tic));
    }

    @Test
    public void testHead() {
        Response response = target().path(Application.ROOT_PATH).request().head();
        assertEquals(200, response.getStatus());
    ...
    public void testFooBarOptions() {
        Response response = target().path(Application.ROOT_PATH).request().header("Accept", "foo/bar").options();
        assertEquals(200, response.getStatus());
    ...
    public void testTextPlainOptions() {
        Response response = target().path(Application.ROOT_PATH).request().header("Accept", MediaType.TEXT_PLAIN).options();
        assertEquals(200, response.getStatus());
        final String allowHeader = response.getHeaderString("Allow");
        _checkAllowContent(allowHeader);
        assertEquals(MediaType.TEXT_PLAIN_TYPE, response.getMediaType());
    ...
    public void testMissingResourceNotFound() {
        ...
        response = target().path(Application.ROOT_PATH + "arbitrary").request().get();
        assertEquals(404, response.getStatus());

        response = target().path(Application.ROOT_PATH).path("arbitrary").request().get();
        assertEquals(404, response.getStatus());
    }
    ...
    public void testLoggingFilterTargetClass() {
        WebTarget target = target().path(Application.ROOT_PATH);
